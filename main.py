...
def kisi_ekle(conn):
    ...
    email = input("E-posta: ")
    adres = input("Adres: ")
    conn.execute("INSERT INTO kisiler (isim, telefon, email, adres) VALUES (?, ?, ?, ?)",
                 (isim, tel, email, adres))
    conn.commit()

def kisileri_listele(conn):
    for row in conn.execute("SELECT * FROM kisiler"):
        print(row)

def kisi_ara(conn):
    kelime = input("Arama: ")
    for row in conn.execute("SELECT * FROM kisiler WHERE isim LIKE ? OR telefon LIKE ? OR email LIKE ? OR adres LIKE ?",
                            (f"%{kelime}%",)*4):
        print(row)

def kisi_sil(conn):
    id_ = input("Silinecek ID: ")
    conn.execute("DELETE FROM kisiler WHERE id=?", (id_,))
    conn.commit()
...
def kisi_guncelle(conn):
    ...
    conn.execute("""UPDATE kisiler SET isim=?, telefon=?, email=?, adres=? WHERE id=?""",
